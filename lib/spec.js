...
module.exports = function(files, options) {
  ...
  if (options.packager && options.packager.length > 0) {
    b.push('Packager: ' + options.packager);
  }
  ...
  if (options.provides && Array.isArray(options.provides) && options.provides.length > 0) {
    b.push('Provides: ' + options.provides.join(', '));
  }
  ...
  if (options.changelog && Array.isArray(options.changelog) && options.changelog.length > 0) {
    ...
    options.changelog
      .forEach(function(entry, index) {
        ...
        b.push('* ' + formatChangelogDate(entry.date) + ' ' + entry.author + ' ' + entry.version);
